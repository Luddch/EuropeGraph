...
void EulerWalk(){
    ...
    int edgeAmount = 0;
    for(int i=0;i<AdjacencyList.size();i++){
        edgeAmount += AdjacencyList[i].edge.size();
    }
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "EulerCycleLength:  " << std::endl;
    std::cout << edgeAmount/2 << std::endl;
    EulerCycle(RUSSIA);
...
void EulerCycle(Europe europe) {
    ...
    while(!AdjacencyList[(int)europe].edge.empty()){
        Europe europe1 = AdjacencyList[(int)europe].edge.back().second;
        AdjacencyList[(int)europe].edge.pop_back();
        for(int i =0;i < AdjacencyList[europe1].edge.size();i++){
            if(AdjacencyList[europe1].edge[i].second == europe){
                AdjacencyList[europe1].edge.erase(AdjacencyList[europe1].edge.begin() + i);
            }
        }
        EulerCycle(europe1);
    }

    std::cout <<counter<<". "<< EuropeOutput[europe] << std::endl;
    counter++;
}
/*
enum Europe{RUSSIA,UKRAINE,BELARUS,ESTONIA,LATVIA,LITHUANIA,FINLAND,NORWAY,SWEDEN,POLAND,MOLDOVA,SLOVAKIA,ROMANIA,CZECHIA,HUNGARY,SERBIA,BULGARIA,
